...
export async function createEvent(data: IEventFormData, file?: File) {
    try {
        ...
        // Append all fields individually so backend receives them correctly
        ...
        // formData.append("image", File.toString());
        if (data.status) {
        ...
        const response = await serverFetch.post("/events/create-event", {
            body: formData,
        });
...
export async function updateEventAction(eventId: string, _prevState: any, formData: FormData) {
    try {
        ...
        if (capacity) data.maxParticipants = Number(capacity);
